...
class EventDAOJ(object):
    ...
    # mongodb에 저장 쿼리
    def save_events(self, link, title, start_date, end_date):

        # saved Flag False로 초기화
        saved = False

        # 기존 jinair collection에 존재하면 저장하지않고 없으면 저장하고 saved Flag True로 변경
        if not self.get_events_by_id(link):
            jinair.insert_one({'link': link, 'title': title, 'start_date': start_date, 'end_date': end_date})
            saved = True
